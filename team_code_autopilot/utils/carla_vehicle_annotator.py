...
def auto_annotate(vehicles, camera, depth_img, max_dist=100, depth_margin=-1, patch_ratio=0.5, resize_ratio=0.5, json_path=None):
    depth_show = False
    bounding_boxes_3d = [get_3d_bouding_box(vehicle, camera) for vehicle in vehicles]
    ...
    return filtered_out, removed_out, bounding_boxes_3d

def auto_annotate_full(vehicles, camera, depth_img, max_dist=100, depth_margin=-1, patch_ratio=0.5, resize_ratio=0.5, json_path=None):
    depth_show = False
    vehicles_transform , vehicles_transform_s = get_list_transform(vehicles, camera)
    vehicles , vehicles_transform , vehicles_transform_s, vehicles_r_dist = filter_distance(vehicles, vehicles_transform, vehicles_transform_s, camera, max_dist)
    bounding_boxes_3d = [get_3d_bouding_box(vehicle, camera) for vehicle in vehicles]
    vehicles , vehicles_transform , vehicles_transform_s, vehicles_r_angle = filter_angle(vehicles, vehicles_transform, vehicles_transform_s, camera)
    bounding_boxes_2d = [get_2d_bb(vehicle, camera) for vehicle in vehicles]
    bounding_boxes_2d_dist = [get_2d_bb(vehicle, camera) for vehicle in vehicles_r_dist]
    bounding_boxes_2d_angle = [get_2d_bb(vehicle, camera) for vehicle in vehicles_r_angle]
    if json_path is not None:
        vehicle_class = get_vehicle_class(vehicles, json_path)
    else:
        vehicle_class = []
    filtered_out, removed_out, _, _ = filter_occlusion_bbox(bounding_boxes_2d, vehicles, camera, depth_img, vehicle_class, depth_show, depth_margin, patch_ratio, resize_ratio)
    return filtered_out, removed_out, bounding_boxes_2d_dist, bounding_boxes_2d_angle, bounding_boxes_3d

def auto_annotate_tf(tfs, camera, depth_img,max_dist=100, depth_margin=-1, patch_ratio=0.5, resize_ratio=0.5, json_path=None ):
    pass 
...
### get bounding box in sensor reference 
def get_3d_bouding_box(vehicle, sensor):
    camera_k_matrix = get_camera_intrinsic(sensor)
    bb_cords = create_bb_points(vehicle)
    cords_x_y_z = vehicle_to_sensor(bb_cords, vehicle, sensor)[:3, :]
    return cords_x_y_z
...
def filter_angle(vehicles_list, v_transform, v_transform_s, sensor):
    ...
    v_angle_1 = np.arctan2(v_transform_s[:,1],v_transform_s[:,0]) * 180 / np.pi #atan2(x,y) -> 여기서는 그럼 v_transform_s[:,0] 고정
    v_angle_2 = np.arctan2(v_transform_s[:,2],v_transform_s[:,0]) * 180 / np.pi
    #selector = np.array(np.absolute(v_angle_1)+np.absolute(v_angle_2) < (int(VIEW_FOV)))
    selector = np.array(np.absolute(v_angle_1) < (int(VIEW_FOV/2)))
    vehicles_list_f = [v for v, s in zip(vehicles_list, selector) if s]
    vehicles_list_r_angle = [v for v, s in zip(vehicles_list, selector) if not s]
    v_transform_f = v_transform[selector[:,0],:]
    v_transform_s_f = v_transform_s[selector[:,0],:]
    return vehicles_list_f , v_transform_f , v_transform_s_f, vehicles_list_r_angle
...
def filter_distance(vehicles_list, v_transform, v_transform_s, sensor, max_dist=100):
    ...
    vehicles_list_f = [v for v, s in zip(vehicles_list, selector) if s]
    vehicles_list_r_dist = [v for v, s in zip(vehicles_list, selector) if not s]
    v_transform_f = v_transform[selector,:]
    v_transform_s_f = v_transform_s[selector,:] 
    return vehicles_list_f , v_transform_f , v_transform_s_f, vehicles_list_r_dist
...
def filter_angle_distance(vehicles_list, sensor, max_dist=100):
    ...
    vehicles_list , vehicles_transform , vehicles_transform_s, _ = filter_distance(vehicles_list, vehicles_transform, vehicles_transform_s, sensor, max_dist)
    vehicles_list , vehicles_transform , vehicles_transform_s, _ = filter_angle(vehicles_list, vehicles_transform, vehicles_transform_s, sensor)
    return vehicles_list

def filter_angle_distance_full(vehicles_list, sensor, max_dist=100):
    vehicles_transform , vehicles_transform_s = get_list_transform(vehicles_list, sensor)
    vehicles_list , vehicles_transform , vehicles_transform_s, vehicles_list_r_dist = filter_distance(vehicles_list, vehicles_transform, vehicles_transform_s, sensor, max_dist)
    vehicles_list , vehicles_transform , vehicles_transform_s, vehicles_list_r_angle = filter_angle(vehicles_list, vehicles_transform, vehicles_transform_s, sensor)
    return vehicles_list, vehicles_list_r_dist, vehicles_list_r_angle
...
def filter_occlusion_bbox(bounding_boxes, vehicles, sensor, depth_img, v_class=None, depth_capture=False, depth_margin=-1, patch_ratio=0.5, resize_ratio=0.5):
    ...
    for v, vs, bbox in zip(vehicles,v_transform_s,bounding_boxes):
        ...
        if v1<0 or u1<0:
            import pdb; pdb.set_trace() 
            #차가 occ 걸리는 거는 차 bbox -영역이라그럼 
        depth_patch = np.array(depth_img[v1:v2,u1:u2])
        dist_delta = dist-depth_patch
        s_patch = np.array(abs(dist_delta) < depth_margin)
...
def filter_occlusion_bbox_debug(bounding_boxes, vehicles, sensor, depth_img, v_class=None, depth_capture=False, depth_margin=-1, patch_ratio=0.5, resize_ratio=0.5):
    filtered_bboxes = []
    filtered_vehicles = []
    filtered_v_class = []
    filtered_out = {}
    ...
    for v, vs, bbox in zip(vehicles,v_transform_s,bounding_boxes):
        ...
        wp = int((bbox[1,0]-bbox[0,0])*resize_ratio/2)
        hp = int((bbox[1,1]-bbox[0,1])*resize_ratio/2)
        u1 = min(max(0,uc-wp),1935)
        u2 = min(max(uc+wp,0), 1935)
        v1 = min(max(0, vc-hp), 1215)
        v2 = min(max(vc+hp,0), 1215)
        if v1<0 or u1<0:
            import pdb; pdb.set_trace() 
            #차가 occ 걸리는 거는 차 bbox -영역이라그럼 
        depth_patch = np.array(depth_img[v1:v2,u1:u2])
        dist_delta = dist-depth_patch
        dist_delta_stats.append(dist_delta)
        patch_size_stats.append((v2-v1)*(u2-u1))
        #debug case for human
        #depth is not working. In Bbox patch, more than actual point cloud are attached to the behind the human, not the human body.  
        #for small human, depth is working well.
        #Best solution will be filtering based depth and angle just as vehicle, and give filtering very loose for the human only. 
        s_patch = np.array(abs(dist_delta) < depth_margin)
        s = np.sum(s_patch) > s_patch.shape[0]*patch_ratio
        selector.append(s)
        patches.append(np.array([[u1,v1],[u2,v2]]))
...
def snap_processing(vehiclesActor, worldSnap, debug=False):
    vehicles = []
    if debug:
        import pdb; pdb.set_trace()
    for v in vehiclesActor:
        ...
        if isinstance(v, carla.libcarla.TrafficLight):
            vsnap.state = v.get_state()
