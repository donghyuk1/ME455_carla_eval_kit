...
class AutoPilot(autonomous_agent_local.AutonomousAgent):
    def setup(self, path_to_conf_file, route_index=None):
        ...
        self.vis_base = pathlib.Path("visualize")
        self.vis_base.mkdir(parents=True, exist_ok=True)

        # run_000, run_001 ... 중 가장 큰 인덱스 다음 번호 선택
        existing = sorted([p for p in self.vis_base.glob("run_*") if p.is_dir()])
        next_idx = 0
        if existing:
            try:
        ...
        self.run_dir = self.vis_base / f"run_{next_idx:03d}"
    ...
    def sensors(self):
        return [{
                    ...
                    },
                {
                    ...
                    'x': 1.3, 'y': 0.0, 'z': 2.3,   # 필요시 차량 셋업에 맞춰 수정
                    'roll': 0.0, 'pitch': 0.0, 'yaw': 0.0,
    ...
    def run_step(self, input_data, timestamp):
        ...
        if ('rgb_front' in input_data) and (self.step % 20 == 0):
            img_bgr = input_data['rgb_front'][1][:, :, :3]  # CARLA 카메라: BGR
            out_path = self.run_dir / f"{self.step:06d}.jpg"
